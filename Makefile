...
.PHONY: help up down reset build data seed stats ask app neo4j logs ps shell smoke bootstrap ollama-list ollama-show ollama-warm
...
help:
	...
	@printf "make ask      Pergunta via CLI. Use Q=\"...\" HOPS=2 MODEL=qwen3.6:latest NUM_CTX=16384\n"
...
ask:
	docker compose run --rm -e OLLAMA_MODEL="$(MODEL)" -e OLLAMA_NUM_CTX="$(NUM_CTX)" -e OLLAMA_TIMEOUT="$(TIMEOUT)" app python scripts/ask.py "$(Q)" --hops "$(HOPS)"
...
smoke:
	docker compose run --rm app python scripts/stats.py
	docker compose run --rm -e OLLAMA_MODEL="$(MODEL)" -e OLLAMA_NUM_CTX="$(NUM_CTX)" -e OLLAMA_TIMEOUT="$(TIMEOUT)" app python scripts/ask.py "Como Frodo se conecta a Sauron?" --hops 2 --no-llm
